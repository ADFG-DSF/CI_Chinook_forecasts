...
rm(list = ls())
brood <- read.csv("C:\\Users\\nadecovich\\Documents\\Kenai\\Kenai early 2024\\KenaiEarly2024.csv")
```
...
## Age 5 model is just the 5 year moving average
...
forecast::auto.arima(dat5$ln) # wants to difference, ?????? put arima order from this function into the order below
...
rick6_ar1 <- arima(rick6$model$lnRS, order=c(1,0,0), xreg = rick6$model$S, method = "ML")
AIC(rick6, rick6_ar1)
rick6_ar1
...
#ricker 
tail(brood, 10)
exp(predict(rick6_ar1, 1, newxreg = 2934)[[1]]) * 2934
```
## Age 7 models
...
forecast::auto.arima(rick7$model$lnRS, xreg = rick7$model$S)
rick7_ar1 <- arima(rick7$model$lnRS, order=c(1,0,0), xreg = rick7$model$S, method = "ML")
AIC(rick7, rick7_ar1)
rick7_ar1
...
dat5CI <- dat5[dat5$ryr %in% 2019:2023, c("ryr", "age5", "md5_pred")]
dat6CI <- dat6[dat6$ryr %in% 2019:2023, c("ryr", "age6", "moving_avg_pred")]
...
errors <- 
  ...
  dplyr::mutate(return = age5 + age6 + age7,
                hind = md5_pred  + moving_avg_pred.x + moving_avg_pred.y,
                error = hind - return)
