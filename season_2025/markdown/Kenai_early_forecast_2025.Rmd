...
## 2024 Kenai early Run Forecast

```{r read in data, echo=FALSE}
rm(list = ls())
brood <- read.csv("H:/Chinook Assessment/Kenai Chinook Early Run Brood Table.csv")
```
...
mu_ar1age5 <- arima(dat5$ln, order=c(0,1,1))
summary(mu_ar1age5)
...
forecast::auto.arima(dat6$age6_ln) # wants to difference (did not change for 2024 forecast)
...
#sibling
tail(brood, 10)
# MUST UPDATE and USE LAST AGE 5'S' IN LOG() TO PREDICT NEXT YEAR
pred_sib6 <- predict(sib6, newdata = data.frame(age5_ln = log(1092)), se.fit = TRUE)
exp(pred_sib6$fit)
...
tail(brood, 10)
exp(predict(rick6_ar1, 1, newxreg = 4055)[[1]]) * 4055
```
...
forecast::auto.arima(dat7$age7_ln) # wants to difference, updated to 1,1,0 for 2024
mu_ar1age7 <- arima(dat7$age7_ln, order=c(1,1,0))
summary(mu_ar1age7)
...
tail(brood, 10)
exp(predict(rick7_ar1, 1, newxreg = 6678)[[1]]) * 6678
...
tail(brood,10)
pred_sib7 <- predict(sib7, newdata = data.frame(age6_ln = log(273)), se.fit = TRUE)
exp(pred_sib7$fit)
...
dat5CI <- dat5[dat5$ryr %in% 2020:2024, c("ryr", "age5", "md5_pred")]
dat6CI <- dat6[dat6$ryr %in% 2020:2024, c("ryr", "age6", "univariate_pred")]
dat7CI <- dat7[dat7$ryr %in% 2020:2024, c("ryr", "age7", "sibling_pred")]
...
errors <- 
  ...
  dplyr::mutate(return = age5 + age6 + age7,
                hind= md5_pred + univariate_pred + sibling_pred,
                error = hind - return)
